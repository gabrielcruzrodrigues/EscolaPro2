...
export class StudentsFormComponent implements AfterViewInit, OnChanges, OnInit {
  form: FormGroup;
  ...
  step: number = 1;
  stepName: string = 'Etapa estudante : 1';
  firstStep: boolean = true;
  lastStep: boolean = false;
  cep: Cep | null = null;
  isLoading: boolean = false;
  @Output() studentData = new EventEmitter<FormData>();
  ...
  studentFinancialResponsibleErrors: string[] = [];

  //modal configs
  isModalOpen: boolean = false;
  modalDeleteMessage: string = "Deseja deletar este documento do registro do estudante?"
  ...
  constructor(
    ...
    this.form = this.fb.group({
      ...
      studentFinancialResponsible: ['', Validators.required],
      role: [100]
    });
  }

  createStudentFormGroupFromStudent(student: Student): FormGroup {
    return this.fb.group({
      ...
      city: [student.city],
      cpf: [student.cpf],
      cpfFile: [student.cpfFile],
      cpfFilePath: [student.cpfFilePath],
      dateOfBirth: [student.dateOfBirth],
      email: [student.email],
      financialFile: [student.financialFile],
      proofOfResidenceFilePath: [student.proofOfResidenceFilePath],
      homeNumber: [student.homeNumber],
      image: [student.image],
      name: [student.name],
      nationality: [student.nationality],
      naturalness: [student.naturalness],
      neighborhood: [student.neighborhood],
      phone: [student.phone],
      rg: [student.rg],
      rgDispatched: [student.rgDispatched],
      rgDispatchedDate: [student.rgDispatchedDate],
      rgFile: [student.rgFile],
      rgFilePath: [student.rgFilePath],
      sex: [student.sex],
      state: [student.state],
      ...
      FixedHealth: [student.FixedHealth],
      studentFinancialResponsible: [student.studentFinancialResponsible]
    });
  }
  ...
  onConfirmDelete(verify: boolean): void {
    if (verify) {
      ...
      switch (this.documentOptionForDelete) {
        case 'rg':
          if (this.studentForEditData)
            this.studentForEditData.rgFilePath = '';
          this.rgFileUploaded = false;
          this.rgDeletedConfirm = true;
          break;
        case 'cpf':
          if (this.studentForEditData)
            this.studentForEditData.cpfFilePath = '';
          this.cpfUploaded = false;
          this.cpfDeletedConfirm = true;
          break;
        case 'financial':
          if (this.studentForEditData)
            this.studentForEditData.proofOfResidenceFilePath = '';
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (this.forEdit && changes['studentForEditData'] && changes['studentForEditData'].currentValue) {
      const data = changes['studentForEditData'].currentValue as Student;
      this.previewUrl = data.image;
      this.FileForShowOfTheStudent(data);
  ...
  downloadFile(option: string): void {
    ...
    switch (option) {
      case 'rg':
        if (this.studentForEditData) {
          link.href = this.studentForEditData.rgFilePath;
        }
        break;
      case 'cpf':
        if (this.studentForEditData) {
          link.href = this.studentForEditData.cpfFilePath;
        }
        break;
      case 'financial':
        if (this.studentForEditData) {
          link.href = this.studentForEditData.proofOfResidenceFilePath;
        }
        break;
    }
  ...
  sendDataFromFatherComponent(): void {
    ...
    formData.append('Id', String(this.studentForEditData?.id));
    ...
    formData.append('studentFinancialResponsible', this.form.get('studentFinancialResponsible')?.value.toString());
    ...
    this.studentData.emit(formData);
  }
  ...
  validateForm(): void {
    ...
    this.getEmailErrors();
    this.getTypeErrors();
    this.getStudentFinancialResponsibleErrors();
  }
  ...
  FileForShowOfTheStudent(student: Student) {
    if (student.rgFilePath != "") {
      this.form.patchValue({ rgFile: student.rgFilePath });
      this.rgFileUploaded = true;
    }

    if (student.cpfFilePath != "") {
      this.form.patchValue({ rgFile: student.cpfFilePath });
      this.cpfUploaded = true;
    }

    if (student.proofOfResidenceFilePath != "") {
      this.form.patchValue({ rgFile: student.proofOfResidenceFilePath });
  ...
  clearInputErrors(option: string): void {
    ...
    if (option == 'studentFinancialResponsible') {
      this.studentFinancialResponsibleErrors = [];
    }
  ...
  getStudentFinancialResponsibleErrors(): void {
    const control = this.form.get('studentFinancialResponsible');
    const errors: string[] = [];

    if (control?.hasError('required')) {
      errors.push('Confirme se o estudante é o responsável financeiro!');
      this.studentFinancialResponsibleErrors = errors;
      return;
    }
  }
